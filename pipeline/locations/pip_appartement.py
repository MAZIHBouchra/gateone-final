...
DATA_PATH  = BASE_DIR / "data/marrakech_immo_location/appartement_location.csv"
MODEL_PATH = BASE_DIR / "model_training/models/xgb_appartement_location.pkl"
...
# ─────────────────────────────────────────────
ZONE_MAP = [
    ("Médina",             r"m[eé]dina|ancienne.m[eé]dina|kasbah|mellah|bab.doukkala|jemaa|appartement.zitoun|dar.el.bacha"),
...
LOC_FINE_MAP = [
    ...
    ("appartement_zitoun",    r"appartement.zitoun"),
...
def load_data(path: Path = DATA_PATH) -> pd.DataFrame:
    ...
    df = df[df["type_bien"].isin(["Appartement", "Location Appartement"])].copy()
    print(f"   Appartements : {len(df)}")
    ...
    df = df[df["surface_num"].between(20, 500)].copy()
    df["_pm2"] = df["prix_num"] / df["surface_num"]
    df = df[df["_pm2"].between(20, 2_000)].copy()
    ...
    df["segment_prix"] = pd.cut(
        df["prix_num"],
        bins=[0, 4_000, 8_000, 15_000, 1e12],
...
def split_and_encode(df: pd.DataFrame, test_size: float = 0.2, seed: int = 42, random_state: int = None):
    ...
    def _enrich(X, src_df):
        ...
        if "segment_prix" in src_df.columns:
            ...
            prix_est = surf * pm2_l
            X["segment_prix"] = pd.cut(prix_est, bins=[0, 4_000, 8_000, 15_000, 1e12], labels=["eco","mid","premium","ultra"]).astype(str).fillna("mid")
...
def evaluate(pipeline, X_test, y_test_or_dftest=None, df_test=None,
    ...
    print("\n" + "═" * 45)
    print("  MÉTRIQUES — APPARTEMENT LOCATION")
    print("═" * 45)
...
def plot_results(pipeline, X_test, y_test_or_dftest=None, metrics=None, save_dir=None):
    ...
    fig.suptitle(f"Appartements — R²={metrics['R²']:.3f} | MAPE={metrics['MAPE (%)']:.1f}%",
    ...
    if save_dir:
        ...
        fig.savefig(Path(save_dir) / "appartement_results.png", dpi=150, bbox_inches="tight")
    plt.show()
...
def run_pipeline(tune=True, n_trials=150):
    print("\n" + "═" * 60)
    print("  PIPELINE APPARTEMENT LOCATION")
    print("═" * 60)
