...
DATA_PATH  = BASE_DIR / "data/marrakech_immo_vente/maison_vente.csv"
MODEL_PATH = BASE_DIR / "model_training/models/xgb_maison_vente.pkl"
...
def load_data(path: Path = DATA_PATH) -> pd.DataFrame:
    ...
    # ── Filtres ──
    df = df[df["prix_num"].between(200_000, 200_000_000)].copy()
    df = df[df["surface_num"].between(20, 5_000)].copy()
    df["_pm2"] = df["prix_num"] / df["surface_num"]
    df = df[df["_pm2"].between(500, 100_000)].copy()
    ...
    df["segment_prix"] = pd.cut(
        df["prix_num"],
        bins=[0, 800_000, 2_000_000, 5_000_000, 1e12],
...
def predict_price(pipeline, bien_dict: dict, stats: dict) -> dict:
    ...
    t["segment_prix"] = ("eco"     if prix_est < 800_000  else
                         "mid"     if prix_est < 2_000_000 else
