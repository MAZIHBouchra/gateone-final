...
DATA_PATH  = BASE_DIR / "data/marrakech_immo_vente/bureaux_vente.csv"
MODEL_PATH = BASE_DIR / "model_training/models/xgb_bureaux_vente.pkl"
...
NUMERIC_FEATURES = [
    # Surface
    "surface_num", "log_surface", "surface_par_piece", 
    # Pièces
    ...
    "surf_x_standing",
    # NLP keywords
    "kw_open_space", "kw_centre_affaires", "kw_standing", "kw_neuf", "kw_titre", "kw_renove", "kw_ascenseur", "kw_parking",
...
ZONE_MAP = [
    ("Guéliz",             r"gueliz|guéliz|hassan|mohamed.v|abdelkrim"),
    ("Hivernage",          r"hivernage|menara"),
    ("Route de Casablanca",r"casablanca"),
    ("Route de Safi",      r"safi"),
    ("Targa",              r"targa"),
    ("Massira",            r"massira|azli|sokoma|socoma"),
    ("Sidi Ghanem",        r"sidi.ghanem|industriel"),
    ("Médina",             r"m[eé]dina|ancienne.m[eé]dina|kasbah|mellah|bab.doukkala|jemaa"),
    ("Issil/Majorelle",    r"issil|majorelle|allal.fassi|boukar"),
]

LOC_FINE_MAP = [
    ("gueliz_centre",  r"gueliz|guéliz|hassan|mohamed.v"),
    ("hivernage",      r"hivernage"),
    ("allal_fassi",    r"allal.fassi|boukar|majorelle"),
    ("route_casa",     r"route.de.casablanca|casablanca"),
    ("route_safi",     r"route.de.safi|safi"),
    ("sidi_ghanem",    r"sidi.ghanem"),
    ("targa",          r"targa"),
    ("massira",        r"massira"),
    ("medina",         r"m[eé]dina|ancienne.m[eé]dina"),
]
...
def extract_keywords(titre, description=""):
    ...
    return {
        "kw_open_space":      int(bool(re.search(r"open.space|plateau", text))),
        "kw_centre_affaires": int(bool(re.search(r"centre.d.?affaire|business.center", text))),
        "kw_standing":        int(bool(re.search(r"haut.standing|luxe|prestige", text))),
        "kw_neuf":            int(bool(re.search(r"neuf|neuve|jamais.habit", text))),
        "kw_titre":           int(bool(re.search(r"titre.foncier|melk|tf\b", text))),
        "kw_renove":          int(bool(re.search(r"rénov|renov|refait|restaur", text))),
        "kw_ascenseur":       int(bool(re.search(r"ascenseur", text))),
        "kw_parking":         int(bool(re.search(r"parking|sous.sol", text))),
    }
...
def load_data(path: Path = DATA_PATH) -> pd.DataFrame:
    ...
    df = df[df["_pm2"].between(2_000, 150_000)].copy()
    ...
    df["nb_pieces"]         = pd.to_numeric(df.get("nb_pieces"), errors="coerce").fillna(df["chambres_num"] + df["salles_bain_num"]).clip(1, 100)
...
def split_and_encode(df: pd.DataFrame, test_size: float = 0.2, seed: int = 42, random_state: int = None):
    ...
    def _enrich(X, src_df):
        ...
        if "segment_prix" in src_df.columns:
            ...
            prix_est = surf * pm2_l
            X["segment_prix"] = pd.cut(prix_est, bins=[0, 600_000, 1_500_000, 5_000_000, 1e12], labels=["eco","mid","premium","ultra"]).astype(str).fillna("mid")
...
def plot_results(pipeline, X_test, y_test_or_dftest=None, metrics=None, save_dir=None):
    ...
    fig.suptitle(f"Bureaux — R²={metrics['R²']:.3f} | MAPE={metrics['MAPE (%)']:.1f}%",
                 fontsize=14, fontweight="bold")

    ax = axes[0, 0]
    ax.scatter(prix_reel / 1e6, prix_pred / 1e6, alpha=0.4, s=15, color="#2563eb")
    ...
    ax = axes[1, 0]
    ax.scatter(prix_reel / 1e6, (prix_pred - prix_reel) / 1e6, alpha=0.3, s=15, color="#059669")
    ax.axhline(0, color="red", linestyle="--")
    ax.set_xlabel("Prix réel (M MAD)"); ax.set_ylabel("Résidu (M MAD)")
    ...
    if save_dir:
        ...
        fig.savefig(Path(save_dir) / "bureaux_results.png", dpi=150, bbox_inches="tight")
    plt.show()
...
def predict_price(pipeline, bien_dict: dict, stats: dict) -> dict:
    ...
    city_pm2   = stats.get("city_pm2", 15_000)
    f_lp       = stats.get("f_lp", np.log(1_600_000))
    ...
    t["chambres_num"]     = float(b.get("chambres_num", 0))
    t["salles_bain_num"]  = float(b.get("salles_bain_num", 1))
    t["nb_pieces"]        = float(b.get("nb_pieces", t["chambres_num"].iloc[0] + t["salles_bain_num"].iloc[0]))
    t["surface_par_piece"] = s / max(t["nb_pieces"].iloc[0], 1)
    ...
    t["score_standing"]   = t["ascenseur"]*3 + t["climatisation"]*2 + t["securite"]*2 + t["parking"]*2 + t["neuf"]*2
    t["score_confort"]    = t["meuble"] + t["terrasse"]
    t["nb_equipements"]   = sum(t[c].iloc[0] for c in ["ascenseur","parking","terrasse","climatisation","securite","meuble","neuf","jardin"])
    ...
    t["segment_prix"] = ("eco"     if prix_est < 600_000  else
                         "mid"     if prix_est < 1_500_000 else
                         "premium" if prix_est < 5_000_000 else "ultra")
    t["cat_surface"]  = ("tiny"   if s < 40  else "small"  if s < 80 else
                         "medium" if s < 150 else "large"  if s < 300 else "estate")
    t["localisation_fine"] = loc

    for col in ["kw_open_space", "kw_centre_affaires", "kw_standing", "kw_neuf", "kw_titre", "kw_renove", "kw_ascenseur", "kw_parking"]:
        t[col] = int(b.get(col, 0))
    ...
    margin     = 0.25 if mode == "range" else None
