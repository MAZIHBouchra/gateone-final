...
DATA_PATH  = BASE_DIR / "data/marrakech_immo_vente/studio_vente.csv"
MODEL_PATH = BASE_DIR / "model_training/models/xgb_studio_vente.pkl"
...
NUMERIC_FEATURES = [
    ...
    # NLP keywords
    "kw_meuble_desc", "kw_renove", "kw_patio", "kw_bassin", "kw_acces",
...
ZONE_MAP = [
    ("Médina",             r"m[eé]dina|ancienne.m[eé]dina|kasbah|mellah|bab.doukkala|jemaa|studio.zitoun|dar.el.bacha"),
...
def extract_keywords(titre, description=""):
    ...
    return {
        "kw_meuble_desc":     int(bool(re.search(r"meublé|meuble|equipé|équipé", text))),
...
def load_data(path: Path = DATA_PATH) -> pd.DataFrame:
    ...
    # Studios uniquement
    df = df[df["type_bien"].isin(["Studio", "Vente Studio"])].copy()
    print(f"   Studios : {len(df)}")
    ...
    # ── Filtres ──
    df = df[df["prix_num"].between(300_000, 100_000_000)].copy()
    ...
    df["segment_prix"] = pd.cut(
        df["prix_num"],
        bins=[0, 1_500_000, 3_000_000, 8_000_000, 1e12],
...
def split_and_encode(df: pd.DataFrame, test_size: float = 0.2, seed: int = 42, random_state: int = None):
    ...
    def _enrich(X, src_df):
        ...
        if "segment_prix" in src_df.columns:
            ...
            prix_est = surf * pm2_l
            X["segment_prix"] = pd.cut(prix_est, bins=[0, 1_500_000, 3_000_000, 8_000_000, 1e12], labels=["eco","mid","premium","ultra"]).astype(str).fillna("mid")
...
def plot_results(pipeline, X_test, y_test_or_dftest=None, metrics=None, save_dir=None):
    ...
    fig.suptitle(f"Studios — R²={metrics['R²']:.3f} | MAPE={metrics['MAPE (%)']:.1f}%",
    ...
    if save_dir:
        ...
        fig.savefig(Path(save_dir) / "studio_results.png", dpi=150, bbox_inches="tight")
    plt.show()
...
def predict_price(pipeline, bien_dict: dict, stats: dict) -> dict:
    ...
    city_pm2   = stats.get("city_pm2", 12_500)
    ...
    t["segment_prix"] = ("eco"     if prix_est < 1_500_000  else
                         "mid"     if prix_est < 3_000_000 else
                         "premium" if prix_est < 8_000_000 else "ultra")
    ...
    for col in ["kw_meuble_desc", "kw_renove", "kw_patio", "kw_bassin", "kw_acces",
                "kw_titre", "kw_hammam", "kw_toit"]:
        t[col] = int(b.get(col, 0))
