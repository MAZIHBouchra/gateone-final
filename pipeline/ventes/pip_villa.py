...
def load_data(path: str) -> pd.DataFrame:
    ...
    df = df[df["prix_num"] >= 1_200_000].copy()   # <2M MAPE=64% → monter le seuil réduit le bruit
    ...
    df["segment_prix"] = pd.cut(
        df["prix_num"],
        bins=[0, 2_000_000, 5_000_000, 15_000_000, 1e12],
...
def evaluate(pipeline, X_test, y_test, X_train=None, y_train=None, cv_folds: int = 5):
    ...
    print("\n" + "═" * 45)
    print("  MÉTRIQUES — VILLA VENTE v2")
    print("═" * 45)
...
def plot_results(pipeline, X_test, y_test):
    ...
    ax = axes[0, 1]
    ax.hist(residuals/1e6, bins=60, color="#10B981", edgecolor="white", lw=0.5)
    ax.axvline(0, color="red", ls="--", lw=1.5)
    ax.set_xlabel("Résidu (M MAD)"); ax.set_title("Distribution résidus")
    ax.grid(True, alpha=0.3)

    ax = axes[1, 0]
    ax.scatter(y_pred_mad/1e6, residuals/1e6, alpha=0.4, s=15, color="#F59E0B")
    ax.axhline(0, color="red", ls="--", lw=1.5)
    ax.set_xlabel("Prix prédit (M MAD)"); ax.set_ylabel("Résidu (M MAD)")
...
def predict_price(pipeline, villa: dict, stats: dict) -> float:
    ...
    t["segment_prix"] = ("eco" if pe < 2e6 else
                         "mid" if pe < 5e6 else
                         "premium" if pe < 15e6 else "ultra")
